...
const char * default_beta_peer_network = "peering-beta.maars.com";
const char * default_live_peer_network = "livenet.maars.com";
}
	...
	switch (version_a)
	{
		...
		case 15:
		{
			...
			peers_l.array_entries<std::string> ([&peers](std::string entry) {
				if (entry == "peers-beta.maars.com")
				{
					entry = default_beta_peer_network;
				}
				else if (entry == "livenet.maars.com")
				{
