...
void nano::uint256_union::encode_account (std::string & destination_a) const
{
	...
	destination_a.append ("_aam"); // xrb_
...
bool nano::uint256_union::decode_account (std::string const & source_a)
{
	...
	if (!error)
	{
		auto maa_prefix (source_a[0] == 'm' && source_a[1] == 'a' && source_a[2] == 'a' && (source_a[3] == '_' || source_a[3] == '-'));
		error = (maa_prefix && source_a.size () != 64);
		
		if (!error)
		{
			if (maa_prefix)
			{
				auto i (source_a.begin () + (maa_prefix ? 4 : 5));
				if (*i == '1' || *i == '3')
				{
